...
int maxsubarray(int arr[], int l, int r){
    ...
    int mid = l + (r - l) / 2;
    int maxleft = maxsubarray(arr, l, mid);
    int maxright = maxsubarray(arr, mid+1, r);
    int corssmax = maxsubarraycross(arr, l, mid, r);

    return max(maxleft, max(maxright, corssmax));
}
